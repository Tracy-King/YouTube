...
class TemporalAttentionLayer(torch.nn.Module):
  ...
  def __init__(self, n_node_features, n_node_old_embedding, n_neighbors_features, n_edge_features, time_dim,
    ...
    self.rnn = nn.RNN(self.query_dim, self.embed_dim, 1)
  ...
  def forward(self, src_node_features, src_node_old_embedding, src_time_features, neighbors_features,
    ...
    #print('query:', (torch.isfinite(query)==False).nonzero().shape[0])
    #print('key:', (torch.isfinite(key)==False).nonzero().shape[0])
...
class TemporalAttentionLayerOrigin(torch.nn.Module):
  """
  Temporal attention layer. Return the temporal embedding of a node given the node itself,
   its neighbors and the edge timestamps.
  """
  ...
  def __init__(self, n_node_features, n_neighbors_features, n_edge_features, time_dim,
               output_dimension, n_head=2,
               dropout=0.1):
    super(TemporalAttentionLayerOrigin, self).__init__()
