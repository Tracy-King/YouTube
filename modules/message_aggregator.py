...
class LastMessageAggregator(MessageAggregator):    # last（neighbor messages）
  def __init__(self, device):
    super(LastMessageAggregator, self).__init__(device)

  def aggregate(self, node_ids, messages):    # node_ids:list    message:dict
    ...
    for node_id in unique_node_ids:
        if len(messages[node_id]) > 0:
            ...
            unique_messages.append(messages[node_id][-1][0])   # [-1] 代表last message
...
class MeanMessageAggregator(MessageAggregator):
  ...
  def aggregate(self, node_ids, messages):      # mean（neighbor messages）
    """Only keep the last message for each node"""
    unique_node_ids = np.unique(node_ids)
    unique_messages = []
    unique_timestamps = []
    ...
    for node_id in unique_node_ids:
      if len(messages[node_id]) > 0:
        ...
        to_update_node_ids.append(node_id)
        unique_messages.append(torch.mean(torch.stack([m[0] for m in messages[node_id]]), dim=0))  # mean 代表mean message
        unique_timestamps.append(messages[node_id][-1][1])
